...
int main (int argc, char **argv) {
	...
	if ( inet_pton(AF_INET, argv[1], &(myhost.sin_addr)) != 1 ) {
		log_message(LOG_CONSOLE, _("Invalid host address"));
		exit(EXIT_FAILURE);
	}
	...
	if ( setsockopt(sock_listen, SOL_SOCKET, SO_REUSEADDR, &yes, sizeof(int)) ) {
		log_error(_("Error setsockopt()"));
		exit(EXIT_FAILURE);
	}
	
	if ( bind(sock_listen, (struct sockaddr*) &myhost, sizeof(myhost)) ) {
		log_error(_("Error bind()"));
		exit(EXIT_FAILURE);
	}
	
	if ( listen(sock_listen, BACKLOG) ) {
		log_error(_("Error listen()"));
		exit(EXIT_FAILURE);
	}
	
	
	inet_ntop(AF_INET, &(myhost.sin_addr), buffer, INET_ADDRSTRLEN);
	flog_message(LOG_INFO, _("Server listening on %s:%hu"), buffer, ntohs(myhost.sin_port));
			...
			if ( FD_ISSET(i, &_readfds) ) {
				if ( i == sock_listen ) {
					...
					if ( client != NULL && client->read_dispatch != NULL ) {
						...
						flog_message(LOG_WARNING, _("Unexpected event on line %d, got read event on socket descriptor %d"), __LINE__, i);
				...
				if ( client != NULL && client->read_dispatch != NULL ) {
					...
					flog_message(LOG_WARNING, _("Unexpected event on line %d, got write event on socket descriptor %d"), __LINE__, i);
					unmonitor_socket_r(sock_client);
					unmonitor_socket_w(sock_client);
					shutdown(sock_client, SHUT_RDWR);
					close(sock_client);
	...
	flog_message(LOG_DEBUG, _("Just exited main while loop with sel_status=%d"), sel_status);
	...
	if ( sel_status == 0 ) {
		log_message(LOG_INFO, _("Timeout. Exit."));
		exit(EXIT_SUCCESS);
	} else {
...
void accept_connection() {
	...
	log_message(LOG_DEBUG, _("Going to accept a new connection..."));
	if ( (sock_client = accept(sock_listen, (struct sockaddr*) &yourhost, &addrlen)) >= 0 ) {
		...
		flog_message(LOG_INFO, _("Incoming connection from %s"), client_sockaddr_p(client));
...
void get_username(struct client_node *client) {
	...
	if (received != 1) {
		flog_message(LOG_DEBUG, _("Received=%d on line %d from %s"), received, __LINE__, client_canon_p(client));
		client_disconnected(client);
		return;
	}
	
	if (cmd != REQ_LOGIN) {
		flog_message(LOG_WARNING, _("Got BADREQ on line %d, cmd=%s from %s"), __LINE__, magic_name(cmd), client_canon_p(client));
		send_byte(client, RESP_BADREQ);
		return;
	}

	flog_message(LOG_DEBUG, _("Got REQ_LOGIN from %s"), client_sockaddr_p(client));
		
	received = recv(client->socket, &(client->username_len), 1, 0); /*FIXME use select(_readfds) */

	if ( received == 1 ) {
		if ( client->username_len > MAX_USERNAME_LENGTH ) {
			flog_message(LOG_INFO_VERBOSE, _("Client %s tried to login with invalid username"), client_sockaddr_p(client));
		...
		if ( received == client->username_len + 2 ) {
			...
			if ( !username_is_valid(client->username, client->username_len) ) {
				...
				flog_message(LOG_INFO_VERBOSE, _("Client %s tried to login with existing username=%s"), client_sockaddr_p(client), dbl->username);
				send_byte(client, RESP_EXIST);
			} else {
				inet_ntop(AF_INET, &(client->addr.sin_addr), buffer, INET_ADDRSTRLEN);
				flog_message(LOG_INFO, _("Client %s has username [%s]"), client_sockaddr_p(client), client->username);
				flog_message(LOG_INFO, _("[%s] Listening on %s:%hu (udp)"), client->username, buffer, client->udp_port);
			...
			flog_message(LOG_WARNING, _("Received=%d on line %d from %s"), received, __LINE__, client_canon_p(client));
			client_disconnected(client);
		}
	} else {
		flog_message(LOG_WARNING, _("Received=%d on line %d from %s"), received, __LINE__, client_canon_p(client));
...
void idle_free(struct client_node *client) {
	...
	flog_message(LOG_DEBUG, _("Got cmd=%s from %s in idle_free"), magic_name(cmd), client_canon_p(client));
	...
	switch ( cmd ) {
		...
		case REQ_PLAY:
			...
			if ( received == 1 ) {
				if ( length > MAX_USERNAME_LENGTH ) {
					flog_message(LOG_WARNING, _("[%s] requested to play with nonexistent player (too long)"), client->username);
				...
				if ( received == length ) {
					...
					if ( opp == NULL ) {
						flog_message(LOG_INFO_VERBOSE, _("[%s] requested to play with nonexistent player"), client->username);
						send_byte(client, RESP_NONEXIST);
					} else if ( opp == client ) {
						flog_message(LOG_INFO_VERBOSE, _("[%s] requested to play with himself"), client->username);
						send_byte(client, RESP_NONEXIST);
					} else if ( opp->state != FREE ) {
						flog_message(LOG_INFO_VERBOSE, _("[%s] requested to play with non-FREE player %s"), client->username, client_canon_p(opp));
		...
		default:
			flog_message(LOG_WARNING, _("Unexpected request from %s in idle_free"), client_canon_p(client));
			send_byte(client, RESP_BADREQ);
	}
}
...
void idle_play(struct client_node *client) {
	...
	flog_message(LOG_DEBUG, _("Got cmd=%s from %s in idle_play"), magic_name(cmd), client_canon_p(client));
	...
	switch ( cmd ) {
		...
		case REQ_END:
			if ( client->play_with != NULL ) flog_message(LOG_INFO, _("[%s] stopped playing with [%s]"), client->username, client->play_with->username);
			else flog_message(LOG_INFO, _("[%s] stopped playing with a disconnected client"), client->username);
			client->state = FREE;
			client->play_with = NULL;
			log_statechange(client);
			send_byte(client, RESP_OK_FREE);
			break;

		default:
			flog_message(LOG_WARNING, _("Unexpected request from %s in idle_play"), client_canon_p(client));
			send_byte(client, RESP_BADREQ);
	}
}
...
void client_disconnected(struct client_node *client) {
	...
	log_message(LOG_DEBUG, _("Going to drop a client..."));
	if ( client->state == BUSY ) {
		if ( client->req_from != NULL ) {
			opp = client->req_from;
			flog_message(LOG_INFO_VERBOSE, _("[%s] had a play request from [%s]"), client->username, opp->username);
			...
			opp = client->req_to;
			flog_message(LOG_INFO_VERBOSE, _("[%s] had requested to play with [%s]"), client->username, opp->username);
			...
			flog_message(LOG_WARNING, _("%s has unconsistent data on line %d"), client_canon_p(client), __LINE__);
		}
	} else if ( client->state == PLAY ) {
		opp = client->play_with;
		if ( opp != NULL ) {
			flog_message(LOG_INFO_VERBOSE, _("[%s] was playing with [%s]"), client->username, opp->username);
			opp->play_with = NULL;
		} else {
			flog_message(LOG_INFO_VERBOSE, _("[%s] was playing with [[unknown]]"), client->username);
		}
	}

	if ( client->state != NONE && client->state != CONNECTED )
		flog_message(LOG_INFO, _("[%s] disconnected"), client->username);
	else
		flog_message(LOG_INFO, _("[[unknown]] %s disconnected"), client_sockaddr_p(client));
...
void send_byte(struct client_node *client, uint8_t byte) {
	flog_message(LOG_DEBUG, _("Preparing to send %s to %s"), magic_name(byte), client_canon_p(client));
...
void send_data(struct client_node *client) {
	...
	if ( sent > 0 ) {
		...
		if ( client->data_cursor == client->data_count ) {
			flog_message(LOG_DEBUG, _("Finished sending %d bytes of data to %s"), client->data_count, client_canon_p(client));
			if ( client->data != NULL ) {
				flog_message(LOG_DEBUG, _("Freeing %d bytes on line %d"), client->data_count, __LINE__);
				free(client->data);
				client->data = NULL;
			}
			
			flog_message(LOG_DEBUG, _("%s is %s"), client_canon_p(client), state_name(client->state));
			switch ( client->state ) {
				...
				case NONE:
					flog_message(LOG_WARNING, _("%s is NONE on line %d"), client_canon_p(client), __LINE__);
					client_disconnected(client);
					return;
			}
		...
		flog_message(LOG_WARNING, _("Sent=%d on line %d to %s"), sent, __LINE__, client_canon_p(client));
		client_disconnected(client);
	}
}
...
void server_shell() {
	...
	if ( strcmp(buffer, "help" ) == 0 || strcmp(buffer, "?") == 0 ) {
		log_message(LOG_CONSOLE, _("Commands: help, who, playing, exit"));
	} else if ( strcmp(buffer, "who") == 0 ) {
		struct client_node *cn;
		
		if (client_list.count == 0) {
			log_message(LOG_CONSOLE, _("There are no connected clients."));
		} else {
			console->prompt = FALSE;
			flog_message(LOG_CONSOLE, _("There are %d connected clients:"), client_list.count);
			for ( cn = client_list.head; cn != NULL; cn = cn->next ) {
				if ( cn->state == NONE || cn->state == CONNECTED )
					flog_message(LOG_CONSOLE, _("[[unknown]] Host %s, not logged in"), client_sockaddr_p(cn));
				else
					flog_message(LOG_CONSOLE, _("[%s] Host %s listening on %hu"), cn->username, client_sockaddr_p(cn), cn->udp_port);
			}
			console->prompt = '>';
			prompt(>);
		}
			...
			if ( cn->state == PLAY && cn->play_with != NULL ) {
				...
				if ( opp->state == PLAY && opp->play_with == cn ) {
					...
					if ( strcmp(cn->username, opp->username) < 0 )
						flog_message(LOG_CONSOLE, _("[%s] is playing with [%s]"), cn->username, opp->username);
				}
			}
		}
		if ( !playing ) log_message(LOG_CONSOLE, _("No one is playing"));
		console->prompt = '>';
		prompt(>);
	} else if ( strcmp(buffer, "exit") == 0 ) {
		int i;
		flog_message(LOG_INFO_VERBOSE, _("Closing %d client connections..."), client_list.count);
		for ( i = 0; i <= maxfds; i++ ) {
			if ( i == STDIN_FILENO ) continue;
			if ( FD_ISSET(i, &readfds) || FD_ISSET(i, &writefds) ) {
				shutdown(i, SHUT_RDWR);
		...
		log_message(LOG_DEBUG, _("Freeing data structures"));
		destroy_client_list(client_list.head);
		log_message(LOG_CONSOLE, _("Exiting..."));
		exit(EXIT_SUCCESS);
...
void send_client_list(struct client_node *client) {
	...
	flog_message(LOG_INFO_VERBOSE, _("[%s] requested the list of connected clients"), client->username);
	for (cn = client_list.head; cn != NULL; cn = cn->next ) {
		if ( cn->state != NONE && cn->state != CONNECTED ) {
			count++;
	...
	flog_message(LOG_DEBUG, _("Allocating %d bytes on line %d"), total_length, __LINE__);
	...
	flog_message(LOG_DEBUG, _("Preparing to send RESP_WHO data to [%s]"), client->username);
	client->write_dispatch = &send_data;
	monitor_socket_w(client->socket);
	/* client->read_dispatch = &inactive; */
	client->muted = TRUE;
...
void send_play_contact(struct client_node *to, struct client_node *contact) {
	...
	flog_message(LOG_DEBUG, _("Allocating %d bytes on line %d"), to->data_count, __LINE__);
	to->data = malloc(to->data_count);
	check_alloc(to->data);
	pack(to->data, "blw", RESP_OK_PLAY, contact->addr.sin_addr, contact->udp_port);
	to->data_cursor = 0;
	flog_message(LOG_DEBUG, _("Preparing to send RESP_OK_PLAY data to [%s]"), to->username);
...
void send_play_request(struct client_node *from, struct client_node *to) {
	flog_message(LOG_INFO, _("[%s] requested to play with [%s]"), from->username, to->username);
	from->req_to = to;
	to->req_from = from;
	...
	to->data_cursor = 0;
	flog_message(LOG_DEBUG, _("Preparing to send REQ_PLAY data to [%s]"), to->username);
	to->write_dispatch = &send_data;
	monitor_socket_w(to->socket);
...
void get_play_resp(struct client_node *client) {
	...
	if ( resp == RESP_OK_PLAY ) {
		if ( opp == NULL ) {
			...
			flog_message(LOG_INFO_VERBOSE, _("[%s] accepted to play with a disconnected client"), client->username);
		} else {
			flog_message(LOG_INFO, _("[%s] accepted to play with [%s]"), client->username, opp->username);
			send_play_contact(opp, client);
		}
		start_match(client, opp);
	} else if ( resp == RESP_REFUSE ) {
		if ( opp == NULL ) {
			flog_message(LOG_INFO_VERBOSE, _("[%s] refused to play with a disconnected client"), client->username);
		} else {
			flog_message(LOG_INFO, _("[%s] refused to play with [%s]"), client->username, opp->username);
		...
		/* May be some other valid command such as REQ_WHO or REQ_PLAY */
		/* RESP_REFUSE o, per sbaglio, anche RESP_BUSY / PERCHÉ?! */
		flog_message(LOG_WARNING, _("Got %s in get_play_resp from %s"), magic_name(resp), client_canon_p(client));
		/*FIXME what to do? */
	}
}
...
void start_match(struct client_node *a, struct client_node *b) {
	...
	if ( a != NULL && b != NULL )
		flog_message(LOG_INFO, _("[%s] is playing with [%s]"), a->username, b->username);
	else if ( a != NULL )
		flog_message(LOG_INFO_VERBOSE, _("[%s] is playing with a disconnected client"), a->username);
	else if ( b != NULL )
		flog_message(LOG_INFO_VERBOSE, _("[%s] is playing with a disconnected client"), b->username);
	else
		flog_message(LOG_WARNING, _("Unexpected event on line %d, both clients are NULL"), __LINE__);
	/*TODO #__LINE__ */
}
...
void inactive(struct client_node *client) {
	...
	if ( received == 0 ) {
		...
		flog_message(LOG_WARNING, _("Got %s in inactive from %s"), magic_name(buffer[0]), client_canon_p(client));
	} else {
		flog_message(LOG_WARNING, _("Got %d bytes in inactive from %s"), received, client_canon_p(client));
	}
	/*FIXME what to do? */
}
